...
CREATE TABLE IF NOT EXISTS "daemon-tracker_post" (
...
--> statement-breakpoint
DO $$ BEGIN
 ALTER TABLE "daemon-tracker_post" ADD CONSTRAINT "daemon-tracker_post_created_by_daemon-tracker_user_id_fk" FOREIGN KEY ("created_by") REFERENCES "public"."daemon-tracker_user"("id") ON DELETE no action ON UPDATE no action;
EXCEPTION
 WHEN duplicate_object THEN null;
END $$;
--> statement-breakpoint
...
CREATE INDEX IF NOT EXISTS "created_by_idx" ON "daemon-tracker_post" USING btree ("created_by");--> statement-breakpoint
CREATE INDEX IF NOT EXISTS "name_idx" ON "daemon-tracker_post" USING btree ("name");--> statement-breakpoint
